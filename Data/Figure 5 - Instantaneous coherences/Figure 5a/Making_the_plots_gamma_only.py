...
DEFAULT_GAMMAS = [0,1,2,5,10,50,100]
DEFAULT_MEASUREMENTS = [0,250,500,750,1000,1250,1500,1750,2000,3000,4000]
...
def make_color_for_gamma(gamma, cmap, norm):
    ...
    if gamma == 0:
        ...
        start_frac = 0.18   # never use the first 15% of the colormap
        if gamma == 1.:
            start_frac = 0.1
            color = cmap(start_frac + (1.0 - start_frac) * float(norm(gamma)))
        else:
            start_frac = 0.2
            color = cmap(start_frac + (1.0 - start_frac) * 3.5* float(norm(gamma)))
...
def plot_results(results, gammas=DEFAULT_GAMMAS, savepath=None, show=True):
    ...
    # styles to cycle through (markers only; linestyles set per-gamma below)
    marker_list = ['o', '^', '*', 'D', 'o','s' , '.']

    # preserve gamma order and make a unique list in that order
    unique_gammas = list(dict.fromkeys(gammas))

    # build mapping gamma -> (marker, linestyle)
    gamma_style = {}
    for i, g in enumerate(unique_gammas):
        ...
        if i == 0:
            ...
            linestyle = '-' 
        gamma_style[g] = (marker, linestyle)
    ...
    for gamma in gammas:
        ...
        marker_for_gamma, linestyle_for_gamma = gamma_style[gamma]
        ...
        for state_key in ['non_lubricated', 'lubricated']:
            ...
            for (fname, header, x, y) in entries:
                ...
                if gamma == 1 or gamma == 2:
                    mark_number_control = 20
                elif gamma == 0:
                    mark_number_control = 200
                elif gamma == 5 or gamma == 10:
                    mark_number_control = 200
                elif gamma == 50:
                    mark_number_control = 30
                else:
                    mark_number_control = 200
                line, = plt.plot(
                    x2, y2,
                    marker=marker_for_gamma,
                    linestyle=linestyle_for_gamma,
